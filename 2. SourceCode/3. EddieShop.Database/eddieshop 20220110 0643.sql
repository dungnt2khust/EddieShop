...
-- Script date 1/10/2022 6:43:47 AM
...
BEGIN
  ...
  SET @lastRecord = (SELECT
      o.OrderCode
    FROM _order o
    WHERE REGEXP_INSTR(o.OrderCode, CONCAT(@prefix, '[0-9]{', @length, '}'))
...
CREATE TABLE product (
  ...
  Image longblob DEFAULT NULL COMMENT 'Ảnh chính',
...
BEGIN
  SET @lastRecord = (SELECT
      p.ProductCode
    FROM product p
    WHERE REGEXP_INSTR(p.ProductCode, 'SP-[0-9]{4}')
...
CREATE TABLE productdetail (
  ...
  `Like` int(2) DEFAULT 0 COMMENT 'Yêu thích',
  CreatedBy varchar(100) DEFAULT NULL COMMENT 'Người tạo mới',
  ModifiedBy varchar(100) DEFAULT NULL COMMENT 'Người chỉnh sửa',
  CreatedDate datetime DEFAULT NULL COMMENT 'Ngày tạo mới',
  ModifiedDate datetime DEFAULT NULL COMMENT 'Ngày chỉnh sửa',
...
BEGIN
  SELECT
    CONCAT('USER-', LPAD((SUBSTRING((SELECT
        u.Code
      FROM user u
      WHERE REGEXP_INSTR(u.code, 'USER-[0-9]{4}')
...
('1731fa87-79fd-4cc1-6978-553c0310877a', 'username0001', 'User-0001', '12345678@User', 'Nguyễn Tiến Dũng', 0, 'Suscipit et sed. Doloribus consequuntur ea. Dolores eius et. Nihil blanditiis voluptates. Iste fugit similique? Quia qui et. Ut assumenda vero. Natus...', 'user0001@gmail.com', '(465) 098-1888', '64cb9b71-37f4-4244-90b1-0d2b3be2374f', 't4xb-QcPnkCR24daRsncVA', 'vi', 'NTDUNG', '2016-09-21 05:35:43', 'NTDUNG', '1975-01-07 07:45:20'),
('185f84ed-4563-51a0-cac7-6c0aeb6ec302', 'username0002', 'User-0002', '12345678@User', 'Nguyễn Đình Hải Dương', 0, 'Facilis animi a. Fugiat saepe inventore; et eum animi. Ut tenetur animi. Aspernatur et veniam. Quae placeat dolorem. Voluptas dolores unde. Ea vel sit. Sed.', 'user0005@gmail.com', '(784) 105-7292', 'bcada7df-7998-48d9-bca4-2839ea3116ad', 'sCyciOj6u7UM2EscFR6BsA', 'vi', 'NTDUNG', '2002-08-31 23:39:28', 'NTDUNG', '2017-04-17 18:59:15'),
...
--
-- Table eddieshop.product does not contain any data (it is empty)

-- 
-- Dumping data for table _order
--
-- Table eddieshop._order does not contain any data (it is empty)

-- 
-- Dumping data for table productdetail
--
-- Table eddieshop.productdetail does not contain any data (it is empty)

-- 
-- Dumping data for table cart
--
-- Table eddieshop.cart does not contain any data (it is empty)

-- 
-- Dumping data for table admin
--
INSERT INTO admin VALUES
('142cb08f-7c31-21fa-8e90-67245e8b283e', 'admin001', 'ADMIN-001', '12345678@Admin', 'Nguyễn Tiến Dũng', 0, 'Quản trị viên EddieShop', 'dungnt@gmai.com', '0123456789', '84f63884-5a0c-4b6a-aaf9-1acbc638e498', 'vi', 'BX6RfcdJcF6vz_9yobvVrA', 'NTDUNG', '1970-01-01 00:07:13', 'NTDUNG', '1970-01-01 00:00:04'),
('469b3ece-744a-45d5-957d-e8c757976496', 'admin002', 'ADMIN-002', '12345678@Admin', 'Nguyễn Phương Thảo', 1, 'Quản trị viên EddieShop', 'dungnt@gmai.com', '0123456789', 'fdfa7a8d-2fc9-4ce6-bf1f-0ef69379e529', 'vi', '57cdf8c4-47e3-5560-7e41-c', 'NTDUNG', '1986-12-14 19:08:29', 'NTDUNG', '2005-08-18 12:14:49');
